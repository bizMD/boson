...
$(function() {
  ...
  socket.on('correctly answered questions', function(data) {
    console.log(data);
    $('.score').html(data.length);
    return $('.items').html($('.total').html().toNumber() + data.length);
  });
  socket.on('incorrectly answered questions', function(data) {
    console.log(data);
    return $('.items').html($('.total').html().toNumber() + data.length);
  });
  return socket.on('end exam', function() {
    ...
    $('.instructions').remove();
    return $('.congratulations').removeClass('hidden');
  });
});
